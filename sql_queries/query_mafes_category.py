def query_mafe_experiments_category(country, region_code, warehouse, today, days, dataset_id, category):
    ...
    query = f"""
        ...
        (
            ...
            INNER JOIN  lnd_ops.dp_product_catalog cat ON cat.product_id = COALESCE(ns.new_product_id, sales.product_id) AND cat.warehouse = ids.warehouse
        ...
        (
            ...
            INNER JOIN    lnd_ops.dp_product_catalog cat ON cat.product_id = COALESCE(ns.new_product_id, op.product_id) AND cat.warehouse = input_params.warehouse
            WHERE         -- Order Status
                          o.status = 1
        ...
        (
            ...
            INNER JOIN  lnd_ops.dp_product_catalog cat ON cat.product_id = COALESCE(ns.new_product_id, m.product_id) AND cat.warehouse = m.warehouse
            WHERE   date(m.created_on) = '{today}'
                AND m.region_code = '{region_code}'
